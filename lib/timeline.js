...
var Timeline = module.exports = function (opts) {
  ...
  this._apiRoot = opts.apiRoot || API_ROOT;
};
...
Timeline.prototype._request = function(opts, callback) {
  ...
  var request = new Http(reqOpts.url);

  request.onerror = function(err) {
    callback(new Error('HTTP error. Desc: ' + err.error), null, null);
  } 

  request.request(reqOpts, function(res) {
    console.log("request done", res.statusCode);
    if (res.statusCode < 300) {
      return callback(null, res.data, res);
    }

    if (Timeline.errorCodes[res.statusCode]) {
      return callback(new Error(Timeline.errorCodes[res.statusCode]), res.data, res);
    }

    callback(new Error('Unknown error. Status code: ' + res.statusCode), res.data, res);
  });
